...
int main(int argc, char **argv)
{
    ...
    for (int n = 1; n < NSIG; n++)   /* Same handler for all signals */
        (void) signal(n, handler);   /* Ignore errors on SIGKILL and SIGSTOP */
    ...
    if (argc > 1) {
        ...
        char *end;
        int numSecs = strtol(argv[1], &end, 10);
        if (*end != '\0')
            errExit("strtol");
    ...
    for (int n = 1; n < NSIG; n++)   /* Display number of signals received */
        if (sigCnt[n] != 0)
